...
elseif($Step == 1)
{
	...
	$ar = array(
		...
		'TEXT' =>
				...
				<nobr>'.getMsg("ARC_DOWN_URL").'</nobr> <input name=arc_down_url size=40 value="'.htmlspecialchars($arc_down_url).'"><br>
					</div>
				</div>
				<div class=t_div>
	...
	<script>
		function div_show(i)
		{
			...
			strAdditionalParams = '&source=' + arSources[i]; // Если большой пост запрос, до данные GET сохранятся для дальнейшей обработки
		}
		...
		if(count($arDName))
		{
			$ar = array(
				...
				'TEXT' => 
					...
					<p><input type=radio name=db_settings value=skip id=skip '.($_REQUEST['db_settings']=='skip'?'checked':'').'><label for=skip>'.getMsg("DB_SKIP").'</label></p>
...
elseif($Step == 3)
{
	...
	else
	{
		...
		else
		{
			...
			(($rs && mysql_fetch_array($rs)) ? '<div style="color:red">'.getMsg('DOC_ROOT_WARN').'</div>' : '') .
			(file_exists($_SERVER['DOCUMENT_ROOT'].'/.htaccess.restore') ? '<div style="color:red">'.getMsg('HTACCESS_WARN').'</div>' : '') .
			'<input type="hidden" name="arc_name" value="'.htmlspecialchars($arc_name).'">
			<input type="hidden" name="dump_name" id="dump_name_id" value="'. htmlspecialchars($_REQUEST["dump_name"]).'">';
...
function LoadFile($strRequestedUrl, $strFilename, &$strRealUrl)
{
